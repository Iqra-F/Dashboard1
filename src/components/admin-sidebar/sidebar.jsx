  ...
  return (
    ...
    <div className={`w-56 ${state.toggle? 'block':'hidden'} h-full overflow-hidden md:overflow-auto py-6 bg-white  border-r border-neutral-200 flex-col gap-4 inline-flex`}>
      {category.map(({text,icon,url},index)=>{
        return(
          <Link to={`/admin-dashboard/${url}`} key={index} onclick={()=>{
            setActiveCat(text)
          }} className={`self-stretch duration-300 cursor-pointer px-[18px]  ${activeCat===text? 'bg-orange-50 bg-opacity-80 border-r-2 border-orange-600':'text-zinc-500 hover:bg-neutral-200'} py-3.5 items-center gap-3 inline-flex`}>
